...
object sparksubmit {
  ...
  private val sparksubmitOpen:Boolean=PropertiesUtil.prop.getProperty("sparksubmitOpen").toBoolean
  ...
  def getlogPath(splitFile:String): String ={
    val logPath0=PropertiesUtil.prop.getProperty("logPath")+"/"+PropertiesUtil.getRegion(splitFile)+"/"+DateS
